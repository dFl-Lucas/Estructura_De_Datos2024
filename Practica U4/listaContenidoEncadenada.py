...
class ListaEncadenada:
    ...
    def insertar(self, elemento):
        if self.__cant >= 0:
            ...
            if self.__cant == 0:
                ...
                while  aux.getSiguiente() != None and elemento.getRaiz().getFrec() > aux.getDato().getRaiz().getFrec():
                    anterior = aux
                    aux = aux.getSiguiente()
                
                if elemento.getRaiz().getFrec() < aux.getDato().getRaiz().getFrec():
                    anterior.setSiguiente(nuevo_nodo)
                    nuevo_nodo.setSiguiente(aux)
                elif elemento.getRaiz().getFrec() >= aux.getDato().getRaiz().getFrec():
                    aux.setSiguiente(nuevo_nodo)

                self.__cant += 1

    def insertarGPT(self, elemento):
        if self.__cant >= 0:
            nuevo_nodo = Nodo(elemento)
            if self.__cant == 0:  # Caso cuando la lista está vacía
                nuevo_nodo.setSiguiente(self.__cabeza)
                self.__cabeza = nuevo_nodo
            else:
                aux = self.__cabeza
                ...
                while aux is not None and elemento.getRaiz().getFrec() > aux.getDato().getRaiz().getFrec():
                    anterior = aux
                    aux = aux.getSiguiente()

                # Si no hemos avanzado, insertamos al principio
                if anterior is None:
                    nuevo_nodo.setSiguiente(self.__cabeza)
                    self.__cabeza = nuevo_nodo
                else:
    ...
    def recorrer(self):
        ...
        i=0
        while dato is not None:
            print(f'Elemento de la posicion [{i}] --> {dato.getDato().getRaiz()} <--')
            dato = dato.getSiguiente()
            i+=1
